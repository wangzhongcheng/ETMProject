...
public class PrecisionFlowerSize extends TreeFitnessAbstract {
	 ...
	 * @return
	 */
	protected static int countFlowerSize(NAryTree tree, boolean inLoop, int node) {
		//Stop the recursion for leafs
		if (tree.isLeaf(node))
			if (inLoop && tree.getType(node) != NAryTree.TAU)
				return 1;
			else
				return 0;
		...
		if (tree.getType(node) == NAryTree.AND || tree.getType(node) == NAryTree.SEQ)
			inLoop = false;
		/**/
