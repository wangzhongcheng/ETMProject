...
public class SimplicityUselessNodes extends TreeFitnessAbstract {
	...
	public static double uselessNodesRatio(NAryTree tree) {
		double badNodes = 0;
	 ...
	 */
	public static boolean isUselessNode(NAryTree tree, int n) {
		...
		if (tree.getType(n) == NAryTree.TAU
				&& (tree.getType(tree.getParent(n)) == NAryTree.AND || tree.getType(tree.getParent(n)) == NAryTree.SEQ || tree
						.getType(tree.getParent(n)) == NAryTree.REVSEQ)) {
			return true;
		}
		//Or TAUs in 
		...
		if (!tree.isLeaf(n)) {
			...
			for (int c = 0; c < tree.nChildren(n); c++) {
				...
				if (!isUselessNode(tree, tree.getChildAtIndex(n, c))
						&& tree.getType(tree.getChildAtIndex(n, c)) != NAryTree.TAU) {
					allUseless = false;
				}
			}
			if (allUseless)
		...
		if (tree.getType(n) == NAryTree.TAU
				&& (tree.getType(tree.getParent(n)) == NAryTree.XOR || tree.getType(tree.getParent(n)) == NAryTree.OR)) {
			...
			for (int child = 0; tree.getChildAtIndex(par, child) != n; child++) {
				if (tree.getType(tree.getChildAtIndex(par, child)) == NAryTree.TAU
		...
		if (tree.getType(n) == NAryTree.LOOP) {
			...
			for (int i = 0; i < tree.nChildren(n); i++) {
				...
				if (tree.getType(tree.getChildAtIndex(n, i)) == NAryTree.LOOP) {
				...
				} else if (tree.getType(tree.getChildAtIndex(n, i)) == NAryTree.TAU
		...
		if (tree.getNumberOfConfigurations() == 0 && tree.getType(n) != NAryTree.LOOP
