...
public class SimplicityMixed extends TreeFitnessAbstract {
	...
	protected static double nrDuplicateActivities(NAryTree tree) {
		return nrDuplicateActivities(tree, true);
	}

	protected static double nrDuplicateActivities(NAryTree tree, boolean ignoreTaus) {
		...
		do {
			if (classes.contains(tree.getType(i))) {
				...
				if (!(tree.getType(i) == NAryTree.TAU) || !ignoreTaus) {
					duplication++;
				}
			} else {
