...
@KeepInProMCache
@FitnessAnnotation
public abstract class TreeFitnessAbstract implements FitnessEvaluator<NAryTree> {

	//TODO make normalized or absolute value required for all fitness evaluators! e.g. field in this class with methods
			...
			if (dependsOn.length == 0 || clonedMap == null || clonedMap.isEmpty()) {
				...
				for (int i = 0; i < constructors.length; i++) {
					...
					if (correct) {
						...
						return (TreeFitnessAbstract) c.newInstance(args);
					}
				}
			}
