...
public class ParetoFront {
	...
	private TObjectIntMap<NAryTree> treeToIndex = new TObjectIntHashMap<NAryTree>(10, .5f, -1);
	/**
	...
	private ArrayList<NAryTree> indexToTree = new ArrayList<NAryTree>();
	 ...
	 */
	private HashSet<NAryTree> lockedTrees = new HashSet<NAryTree>();

	/**
	 * Set of trees that contain locked trees that should be removed as soon as
	 * the lock is released
	 */
	private HashSet<NAryTree> lockedTreesThatShouldBeRemoved = new HashSet<NAryTree>();
	 ...
	 *            Collection of trees
	 */
	public synchronized void consider(Collection<NAryTree> trees) {
		for (NAryTree t : trees) {
			try {
				consider(t);
	 ...
	 */
	public synchronized boolean consider(NAryTree t) {
		...
		NAryTree normalizedTree = t;

		//For all trees currently in the Pareto front
		for (NAryTree c : treeToIndex.keySet()) {
	 ...
	 *            Tree to add
	 */
	private synchronized void addTree(NAryTree t) {
	 ...
	 */
	public synchronized double compareOnDimension(NAryTree t1, NAryTree t2, TreeFitnessInfo dimension) {
	 ...
	 * @param t
	 */
	private synchronized void removeTree(NAryTree tree) {
		assert checkConsistency();
		...
		for (int j = t; j < indexToTree.size(); j++) {
			...
			NAryTree movingTree = indexToTree.get(j);
	 ...
	 * @return
	 */
	public synchronized int countDominatorsOf(NAryTree t) {
		int nrDominating = 0;

		for (NAryTree candidate : treeToIndex.keySet()) {
	 ...
	 * @return TRUE/FALSE
	 */
	public synchronized boolean dominates(NAryTree t, NAryTree candidate) {
	 ...
	 */
	public synchronized boolean dominates(NAryTree t1, NAryTree t2, boolean requireStrictlyBetter) {
	 ...
	 * @return
	 */
	public synchronized Collection<NAryTree> getFront() {
	 ...
	 */
	public synchronized NAryTree getWorst(TreeFitnessInfo dimension) {
	 ...
	 */
	public synchronized NAryTree getBest(TreeFitnessInfo dimension) {
	 ...
	 */
	public synchronized NAryTree getWorse(TreeFitnessInfo dimension, NAryTree tree) {
		if (!treeToIndex.containsKey(tree)) {
			return null;
		}
	 ...
	 *         provided dimension or tree is not known
	 */
	public synchronized NAryTree getBetter(TreeFitnessInfo dimension, NAryTree tree) {
		if (!treeToIndex.containsKey(tree)) {
			return null;
		}
	 ...
	 *         known
	 */
	public synchronized int getNrBetter(TreeFitnessInfo dimension, NAryTree tree) {
		if (!treeToIndex.containsKey(tree)) {
			return -1;
		}
	 ...
	 *         known
	 */
	public synchronized int getNrWorse(TreeFitnessInfo dimension, NAryTree tree) {
		if (!treeToIndex.containsKey(tree)) {
			return -1;
		}
	 ...
	 * @param tree
	 */
	public synchronized void lockTree(NAryTree tree) {
	 ...
	 */
	public synchronized boolean unlockTree(NAryTree tree) {
	 ...
	 */
	public synchronized boolean inParetoFront(NAryTree tree) {
		NAryTree normalizedTree = tree;
	...
	public synchronized boolean shouldBeRemovedButIsLocked(NAryTree tree) {
		if (treeToIndex.containsKey(tree))
			return lockedTreesThatShouldBeRemoved.contains(tree);
	 ...
	 * @return
	 */
	public synchronized Set<NAryTree> getTreeWithValues(HashMap<TreeFitnessInfo, Double> values) {
		HashSet<NAryTree> trees = new HashSet<NAryTree>();
		//For each tree in the Pareto Front, check the dimension-value and quick fail
		for (NAryTree tree : treeToIndex.keySet()) {
			boolean add = true;
	...
	public synchronized void reduceSize() {
		if (size() > maxSize && maxSize > 0) {
			...
			Iterator<NAryTree> it = trees.iterator();
