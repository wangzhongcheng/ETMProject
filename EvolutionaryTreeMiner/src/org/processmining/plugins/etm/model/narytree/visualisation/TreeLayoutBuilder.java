...
public class TreeLayoutBuilder {
	...
	public TreeLayoutBuilder(NAryTree tree, XEventClasses classes) {
		this.tree = tree;
	...
	protected DefaultPort[] drawNodeOther(GraphLayoutCache g, int node) {
		...
		Type t;
		if (tree.getTypeFast(node) == NAryTree.AND) {
			t = Type.AND;
		} else if (tree.getTypeFast(node) == NAryTree.ILV) {
			t = Type.ILV;
		} else if (tree.getTypeFast(node) == NAryTree.OR) {
			t = Type.OR;
	...
	protected DefaultPort[] drawNodeLeaf(GraphLayoutCache g, int node) {
		...
		if (classes == null) {
			...
			if (tree.getTypeFast(node) == NAryTree.TAU) {
