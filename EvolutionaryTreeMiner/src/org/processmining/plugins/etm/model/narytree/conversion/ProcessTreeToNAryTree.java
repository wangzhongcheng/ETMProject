...
public class ProcessTreeToNAryTree {
	 ...
	 */
	public NAryTree convert(ProcessTree pt) {
	 ...
	 * @return
	 */
	public NAryTree convert(ProcessTree pt, List<ControlFlowConfiguration> configurations) {
	 ...
	 */
	public NAryTree convertNode(Node node, List<ControlFlowConfiguration> configurations) {
		//Quick return tasks/leafs
		if (node instanceof AbstractTask) {
			NAryTree leafTree = new NAryTreeImpl(new int[] { 1 }, new short[] { convertType(node) },
		...
		if (block instanceof AbstractEvent) {
			...
			tree = new NAryTreeImpl(new int[] { 2, 2 }, new short[] { NAryTree.SEQ, convertType(node) }, new int[] {
					NAryTree.NONE, 0 });
		} else {
			//Instantiate tree with only that operator as root
			tree = new NAryTreeImpl(new int[] { 1 }, new short[] { convertType(node) }, new int[] { NAryTree.NONE });
		}
	...
	private static NAryTree applyConfiguration(NAryTree nAryTree, int i, Node node,
		...
		while (nAryTree.getNumberOfConfigurations() < configurations.size()) {
			nAryTree.addConfiguration(new org.processmining.plugins.etm.model.narytree.Configuration(nAryTree.size()));
		}
		...
		for (int c = 0; c < configurations.size(); c++) {
			...
			if (config.isBlocked(inEdge)) {
				nAryTree.setNodeConfiguration(c, i, org.processmining.plugins.etm.model.narytree.Configuration.BLOCKED);
			} else if (config.isHidden(inEdge)) {
				nAryTree.setNodeConfiguration(c, i, org.processmining.plugins.etm.model.narytree.Configuration.HIDDEN);
			} else if (config.getReplacedNodes().containsKey(node.getID())) {
				byte configurationOption = org.processmining.plugins.etm.model.narytree.Configuration.NOTCONFIGURED;
				Type downgradedTo = config.getReplacedNodes().get(node.getID());

				switch (downgradedTo) {
					case AND :
						configurationOption = org.processmining.plugins.etm.model.narytree.Configuration.AND;
						break;
					case XOR :
					case DEF :
						configurationOption = org.processmining.plugins.etm.model.narytree.Configuration.XOR;
						break;
					case SEQ :
						configurationOption = org.processmining.plugins.etm.model.narytree.Configuration.SEQ;
						break;
					case PLACEHOLDER :
					case TIMEOUT :
					case AUTOTASK :
	...
	public static void main(String[] args) {
		...
		NAryTree simpleSkipNAryTree = simpleSkipUS.convert(simpleSkipTree);
		...
		NAryTree exampleNAryTree = exampleUS.convert(exampleTree);
		...
		/*-* /
		NAryTreeImpl t1 = new NAryTreeImpl(new int[] {1}, new short[] {1}, new int[] {NAryTree.NONE});
		System.out.println("NT:" + t1.toString());
		System.out.println(t1.toInternalString());
		NAryTreeImpl t2 = new NAryTreeImpl(new int[] {1}, new short[] {2}, new int[] {NAryTree.NONE});
		System.out.println("NT:" + t2.toString());
		System.out.println(t2.toInternalString());
		NAryTreeImpl tand = new NAryTreeImpl(new int[] {1}, new short[] {NAryTree.AND}, new int[] {NAryTree.NONE});
		System.out.println("NT:" + tand.toString());
		System.out.println(tand.toInternalString());
		tand = tand.add(t1, 0, 0, Integer.MAX_VALUE);
		System.out.println("NT:" + tand.toString());
		System.out.println(tand.toInternalString());
		tand = tand.add(t2, 0, 0, Integer.MAX_VALUE);
		System.out.println("NT:" + tand.toString());
		System.out.println(tand.toInternalString());
		/**/
	}
	...
	/**
	 * Tries to determine the best matching classifier for the given process
	 * tree
	 * 
	 * @param pt
	 *            The process tree for which the classifier needs to be
	 *            determined
	 * @param log
	 *            The original event log (used to create several classifiers to
	 *            test)
	 * @param info
	 * @param classifiers
	 * @return
	 */
	/*-public static XEventClassifier determineUsedClassifier(ProcessTree pt, XLog log, XLogInfo info,
