...
public class TreeUtils {
	...
	public static NAryTree fromString(String s) {
		...
		fromString(chars, intP, map, type, parent, last, NAryTreeImpl.NONE, true);
		NAryTree tree = new NAryTreeImpl(last, type, parent);
	 ...
	 * @param tree
	 */
	private static void configurationsFromString(char[] chars, IntPointer p, NAryTree tree) {
	...
	public static NAryTree fromString(String s, XEventClasses classes) {
	...
	public static NAryTree fromString(String s, TObjectShortMap<String> map) {
		TShortList type = new TShortArrayList();
		TIntList parent = new TIntArrayList();
		TIntList last = new TIntArrayList();
		IntPointer intP = new IntPointer();
		char[] chars = s.toCharArray();
		fromString(chars, intP, map, type, parent, last, NAryTreeImpl.NONE, false);
		NAryTree tree = new NAryTreeImpl(last, type, parent);
		configurationsFromString(chars, intP, tree);
		return tree;
	}

	public static void writeTreeToDot(NAryTree tree, int configurationNumber, OutputStreamWriter out)
			throws IOException {
	...
	private static void writeTreeToDotInternal(NAryTree tree, int configurationNumber, OutputStreamWriter out,
			String... leafLabels) throws IOException {
		// write the header.
		out.write("digraph g{\n");
	...
	private static void writeChildren(NAryTree tree, int node, int configurationNumber, OutputStreamWriter out,
	...
	public static void writeBehaviorToDot(NAryTree tree, int configurationNumber, OutputStreamWriter out,
	 ...
	 * @return
	 */
	public static String toString(NAryTree tree) {
	 ...
	 * @return
	 */
	public static String toString(NAryTree tree, XEventClasses classes) {
	 ...
	 * @return
	 */
	public static String toString(NAryTree tree, int node) {
	 ...
	 * @return
	 */
	public static String toString(NAryTree tree, int node, XEventClasses classes) {
	 ...
	 * @return
	 */
	public static String nodeToString(NAryTree tree, int node) {
	 ...
	 * @return
	 */
	public static String nodeToString(NAryTree tree, int node, XEventClasses classes) {
		if (tree.getType(node) >= 0) {
			// leaf
			if (tree.getType(node) == NAryTree.TAU) {
				return "LEAF: tau";
		...
		} else if (tree.getType(node) == NAryTree.XOR) {
			return "XOR";
		} else if (tree.getType(node) == NAryTree.OR) {
			return "OR";
		} else if (tree.getType(node) == NAryTree.ILV) {
			return "ILV";
		} else if (tree.getType(node) == NAryTree.AND) {
			return "AND";
		} else if (tree.getType(node) == NAryTree.LOOP) {
			return "LOOP";
		} else if (tree.getType(node) == NAryTree.SEQ) {
			return "SEQ";
		} else if (tree.getType(node) == NAryTree.REVSEQ) {
			return "REVSEQ";
		}
		return "";
	}
	 ...
	 * @return
	 */
	public static String nodeToString(NAryTree tree, int node, String[] leafLabels) {
		if (tree.getType(node) >= 0) {
			// leaf
			if (tree.getType(node) == NAryTree.TAU) {
				return "LEAF: tau";
	...
	public static NAryTreeImpl randomTree(int evtClasses, double leafProb, int minNodes, int maxNodes, Random r) {
		NAryTreeImpl tree;
		...
		do {
			...
			last.add(1);
			parent.add(NAryTree.NONE);
			type.add(NAryTree.NONE);
			fillRandom(type, last, parent, 0, r, leafProb, evtClasses, maxNodes);

			tree = new NAryTreeImpl(last, type, parent);
		} while (tree.size() < minNodes);
		return tree;
	}

	private static void fillRandom(TShortList type, TIntList last, TIntList parent, int operator, Random r,
		...
		for (int i = 0; i < children; i++) {
			...
			parent.add(operator);
			last.add(-1);
			fillRandom(type, last, parent, type.size() - 1, r, leafProb, evtClasses, maxNodes);
		}
	...
	public static void main(String[] args) {
		...
		NAryTree tree = randomTree(4, .3, 1, 1);
		printTree(tree);
		String s2 = tree.toString();
		NAryTree treeRead = fromString(s2);
		printTree(treeRead);

		Random r = new Random();
		NAryTreeImpl tree0 = randomTree(5, 0.4, 3, 5);
		...
		if (tree0.size() > 1) {
			...
			NAryTreeImpl tree1 = tree0.swap(node1, node2);
			TreeUtils.printTree(tree1);

			NAryTreeImpl tree2 = randomTree(5, 0.2, 3, 4);
			TreeUtils.printTree(tree2);

			int p = r.nextInt(tree1.size());
			while (tree1.type[p] >= 0 || tree1.type[p] == NAryTree.LOOP) {
			...
			NAryTree tree5 = tree4.swap(node1, node3);
			...
			NAryTree tree6 = tree5.replace(node1, tree4, node3);
	 ...
	 */
	public static NAryTree flatten(NAryTree tree) {
		NAryTree flattenedTree = new NAryTreeImpl(tree);
		...
		for (int i = 0; i < flattenedTree.size(); i++) {
			...
			if (!flattenedTree.isLeaf(i) && flattenedTree.getType(i) != NAryTree.LOOP) {
	 ...
	 */
	public static NAryTree sort(NAryTree tree) {
		NAryTree sortedTree = new NAryTreeImpl(tree);

		for (int i = 0; i < sortedTree.size(); i++) {
			if (!sortedTree.isLeaf(i) && sortedTree.getType(i) != NAryTree.LOOP
					&& sortedTree.getType(i) != NAryTree.SEQ && sortedTree.nChildren(i) > 1) {
				//Bubble sort thingy :)
	...
	public static short getRandomOperatorType(Random rng, int maxType) {
		...
		switch (rng.nextInt(maxType + 1)) {
			...
			case 4 :
				return NAryTree.ILV;
			case 5 :
				return NAryTree.LOOP;
			case 6 :
				return NAryTree.REVSEQ;
			default :
				assert false;
				return NAryTree.NONE; //NEVER REACH HERE
		}
	}
	 ...
	 *         found
	 */
	public static XEventClasses getXEventClassesFromConnection(PluginContext context, NAryTree tree) {
	...
	public static NAryTree rewriteRevSeq(NAryTree tree) {
		NAryTree newTree = new NAryTreeImpl(tree);
		for (int node = 0; node < newTree.size(); node++) {
			if (newTree.getType(node) == NAryTree.REVSEQ) {
				...
				newTree.setType(node, NAryTree.SEQ);
			}
		}
		return newTree;
	}
	 ...
	 */
	public static int getNumberOfNodesConfiguredForConfiguration(NAryTree tree, int configuration) {
