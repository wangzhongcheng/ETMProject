	...
	public static void main(String[] args) throws Exception {
		...
		for (File input : folder.listFiles()) {
			...
			while (line != null) {
				...
				NAryTree tree = TreeUtils.fromString(token);
				...
				cnt = tree.countNodes(NAryTree.AND);
				size -= cnt;
				out.print(cnt);
				out.print(sep);

				cnt = tree.countNodes(NAryTree.XOR);
				size -= cnt;
				out.print(cnt);
				out.print(sep);

				cnt = tree.countNodes(NAryTree.OR);
				size -= cnt;
				out.print(cnt);
				out.print(sep);

				cnt = tree.countNodes(NAryTree.ILV);
				size -= cnt;
				out.print(cnt);
				out.print(sep);

				cnt = tree.countNodes(NAryTree.SEQ) + tree.countNodes(NAryTree.REVSEQ);
				size -= cnt;
				out.print(cnt);
				out.print(sep);

				cnt = tree.countNodes(NAryTree.LOOP);
				size -= cnt;
				out.print(cnt);
				out.print(sep);

				cnt = tree.countNodes(NAryTree.TAU);
				size -= cnt;
				out.print(cnt);
				out.print(sep);
