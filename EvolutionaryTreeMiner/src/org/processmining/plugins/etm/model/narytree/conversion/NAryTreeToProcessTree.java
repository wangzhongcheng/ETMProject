...
public class NAryTreeToProcessTree {
	...
	public static Node convertNode(ProcessTree processTree, Block parentNode, XEventClasses classes, NAryTree tree,
		...
		NAryTree normalizedTree = TreeUtils.rewriteRevSeq(tree);
		...
		if (parentNode == null) {
			...
			for (int c = 0; c < normalizedTree.getNumberOfConfigurations(); c++) {
				if (normalizedTree.getNodeConfiguration(c, node) == org.processmining.plugins.etm.model.narytree.Configuration.BLOCKED
						|| normalizedTree.getNodeConfiguration(c, node) == org.processmining.plugins.etm.model.narytree.Configuration.HIDDEN
						|| normalizedTree.getNodeConfiguration(c, node) == org.processmining.plugins.etm.model.narytree.Configuration.REVSEQ) {
		...
		if (normalizedTree.getNumberOfConfigurations() > 0 && configurations != null && !addPlaceholderRoot) {
			...
			for (int c = 0; c < maxNrConfig; c++) {
				...
				switch (normalizedTree.getNodeConfiguration(c, node)) {
					case org.processmining.plugins.etm.model.narytree.Configuration.NOTCONFIGURED :
						break;
					case org.processmining.plugins.etm.model.narytree.Configuration.BLOCKED :
						...
						break;
					case org.processmining.plugins.etm.model.narytree.Configuration.HIDDEN :
						...
						break;
					//DOWNGRADING:
					case org.processmining.plugins.etm.model.narytree.Configuration.SEQ :
						((Block) newNode).setChangeable(true);
						configuration.setChangeableNode(newNode, Type.SEQ);
						break;
					case org.processmining.plugins.etm.model.narytree.Configuration.AND :
						((Block) newNode).setChangeable(true);
						configuration.setChangeableNode(newNode, Type.AND);
						break;
					case org.processmining.plugins.etm.model.narytree.Configuration.XOR :
						((Block) newNode).setChangeable(true);
						configuration.setChangeableNode(newNode, Type.XOR);
						break;
					case org.processmining.plugins.etm.model.narytree.Configuration.REVSEQ :
						/*
						...
						for (int revseqConfigCounter = 0; revseqConfigCounter < c; revseqConfigCounter++) {
							if (normalizedTree.getNodeConfiguration(revseqConfigCounter, node) == org.processmining.plugins.etm.model.narytree.Configuration.REVSEQ) {
								areFirst = false;
								break; //break once
							}
						}
						...
						if (parentNode != null) {
							...
							NAryTree reversedSequenceSubtree = new NAryTreeImpl(normalizedTree);
							reversedSequenceSubtree.setType(node, NAryTree.REVSEQ);
							...
							for (int revseqDowngradeRemovalConfiguration = 0; revseqDowngradeRemovalConfiguration < reversedSequenceSubtree
								...
								if (reversedSequenceSubtree.getNodeConfiguration(revseqDowngradeRemovalConfiguration,
										node) == org.processmining.plugins.etm.model.narytree.Configuration.REVSEQ) {
									reversedSequenceSubtree.setNodeConfiguration(revseqDowngradeRemovalConfiguration,
											node,
											org.processmining.plugins.etm.model.narytree.Configuration.NOTCONFIGURED);
								}
							}
							...
							for (int revseqConfig = 0; revseqConfig < normalizedTree.getNumberOfConfigurations(); revseqConfig++) {
								if (normalizedTree.getNodeConfiguration(revseqConfig, node) == org.processmining.plugins.etm.model.narytree.Configuration.REVSEQ) {
									...
									configurations.get(revseqConfig).setSelectedReplacement(placeholder,
											revertedSubtree);
								} else if (normalizedTree.getNodeConfiguration(revseqConfig, node) == org.processmining.plugins.etm.model.narytree.Configuration.SEQ
										|| normalizedTree.getNodeConfiguration(revseqConfig, node) == org.processmining.plugins.etm.model.narytree.Configuration.NOTCONFIGURED) {
	...
	public static void addFitnessProperties(ProcessTree processTree, NAryTree naryTree, CentralRegistry registry) {
