...
public abstract class AbstractNAryTreeLPDelegate<T extends Tail> extends AbstractNAryTreeDelegate<T> implements
	...
	public AbstractNAryTreeLPDelegate(AStarAlgorithm algorithm, NAryTree tree, int configurationNumber,
	...
	public AbstractNAryTreeLPDelegate(AStarAlgorithm algorithm, NAryTree tree, int configurationNumber,
		...
		try {
			...
			for (int i = 0; i < tree.size(); i++) {
				...
				if (tree.isLeaf(i) && types[i] != NAryTree.TAU && !tree.isHidden(configurationNumber, i)) {
					leafs++;
				}
				if (types[i] == NAryTree.OR && !tree.isHidden(configurationNumber, i)
						&& tree.getNextFast(i) > tree.getNextFast(i + 1)) {
					ors++;
				}
				if (getType(tree.getParentFast(i)) == NAryTree.OR
			...
			for (int i = 0; i < tree.size(); i++) {
				...
				if (tree.isHidden(configurationNumber, i)) {
					...
					if (p >= 0) {
						if (types[p] == NAryTree.AND || types[p] == NAryTree.SEQ || types[p] == NAryTree.REVSEQ) {
				...
				if (tree.isLeaf(i) || tree.isHidden(configurationNumber, i)) {
					...
					if (tree.isLeaf(i) && !tree.isHidden(configurationNumber, i) && types[i] != NAryTree.TAU) {
						...
						if (getType(tree.getParentFast(i)) == NAryTree.XOR) {
							// -effect on parent's first child's row
	...
	private short getType(int node) {
		return node >= treeSize || node < 0 ? NAryTree.NONE : types[node];
	}
			...
			if (types[i] == NAryTree.OR) {
				if (s == StateBuilder.F) {
					...
					if (!allN) {
						...
						if (i == 0) {
							rhs[1] = 1;
						} else if (types[parent] == NAryTree.XOR) {
							// use the  XOR parent
							rhs[parent + 2] = 1;
						} else {
							rhs[i + 1] = 1;
