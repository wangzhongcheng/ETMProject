...
public class PaperMethods {
	...
	public static String naryTreeToLatex(NAryTree inputTree, XEventClasses eventClasses) {
		return naryTreeToLatex(inputTree, eventClasses, null);
	}

	public static String naryTreeToLatex(NAryTree inputTree, XEventClasses eventClasses, CentralRegistry registry) {
		boolean createForest = inputTree.size() > 20;
		createForest = true;
		NAryTree tree = TreeUtils.rewriteRevSeq(inputTree);
		tree = inputTree;
		...
		if (registry != null && registry.isFitnessKnown(tree)) {
			...
			if (registry instanceof CentralRegistryConfigurable) {
				...
				for (int c = 0; c < registryConf.getNrLogs(); c++) {
					NAryTree configuredTree = tree.applyConfiguration(c);
	...
	private static String nodeToLatex(NAryTree tree, int node, XEventClasses classes) {
	...
	public static String nodeToLatexForest(NAryTree tree, int node, XEventClasses classes) {
		//First get the string representation for this node
		String nodeString = "";
		switch (tree.getType(node)) {
			case NAryTree.XOR :
