...
public class CommandLineInterface {
	...
	public void run() {
		...
		for (int m = 0; m < methods.length; m++) {
			...
			} else if (methods[m].equalsIgnoreCase(BLACK)) {
				blackMethod(loggingPath + BLACK + "/");
			} else if (methods[m].equalsIgnoreCase(GREEN)) {
				greenMethod(loggingPath + GREEN + "/");
	...
	private void normalMethod(String loggingPath) {
		...
		for (int l = 0; l < logs.length; l++) {
			...
			etmMind.run();
			NAryTree treeMind = etmMind.getResult();
	...
	private void paretoMethod(String loggingPath) {
		...
		for (int l = 0; l < logs.length; l++) {
			...
			try {
				Collection<NAryTree> pfTrees = paretoFront.getFront();
				for (NAryTree tree : pfTrees) {
	...
	private void paretoGreenMethod(String loggingPath) {
		...
		Collection<NAryTree> pfTrees = new HashSet<NAryTree>();
		pfTrees.addAll(paretoFront.getFront()); //TODO fix pfTrees is still backed by PF
		Collection<NAryTree> pfTreesNormalized = new HashSet<NAryTree>();
		for (NAryTree tree : pfTrees) {
			NAryTree normalized = TreeUtils.normalize(tree);
			etmParamPareto.getFitnessEvaluator().getFitness(normalized, null);
	...
	private void blueMethod(String loggingPath) {
		...
		NAryTree[] trees = new NAryTree[logs.length];
	...
	private void redMethod(String loggingPath) {
		...
		etmMstar.run();
		NAryTree treeMStar = etmMstar.getResult();
		...
		//And the seed
		NAryTree[] seed = new NAryTree[] { treeMStar };
		...
		for (int l = 0; l < logs.length; l++) {
			...
			TreeFitnessAbstract mainEval = etmParamMind.getFitnessEvaluator();
	...
	private void blackMethod(String loggingPath) {
		...
		if (etmParamMstar.getFitnessEvaluator() instanceof OverallFitness) {
			...
			for (TreeFitnessAbstract key : eval.getEvaluators().keySet()) {
				if (key instanceof FitnessReplay) {
					eval.getEvaluators().put(key, frWeightBlack);
				} else if (key instanceof PrecisionEscEdges) {
	...
	private void greenMethod(String loggingPath) {
		...
		NAryTree result = etm.getResult();

		outputTree(etmParam.getPath(), "GREEN-Mc", result, registry);
	}

	private static NAryTree[] setGreenSeed(CentralRegistry registry) {
		/*
		 * TEMP: start from the ideal tree to see if another tree is better and
		 * why
		 */
		NAryTree perfectTree = TreeUtils.fromString("SEQ( LEAF: A+complete , "
		...
		NAryTree[] seed = new NAryTree[] { perfectTree };
		/*
	 ...
	 * @param trees
	 */
	private void mergeTrees(String loggingPath, String method, XLog[] logs, NAryTree[] trees, CentralRegistry registry) {
		if (trees.length < 2) {
	...
	public ETMParam buildETMParam(String loggingPath, XLog[] logs) {
		...
		AbstractCandidateFactory<NAryTree> factory;
		...
		if (enableMaikelsSmartStuff) {
			...
			evolutionaryOperators.add(new CombinationCrossover<NAryTree>(1, new Probability(crossOverChance), registry,
					null));
	...
	public ETMParamPareto buildETMParetoParam(String loggingPath, XLog[] log) {
		...
		ArrayList<NAryTree> seed = new ArrayList<NAryTree>();
	 ...
	 */
	public static void outputTree(String outputDir, String key, NAryTree tree, CentralRegistry centralRegistry) {
