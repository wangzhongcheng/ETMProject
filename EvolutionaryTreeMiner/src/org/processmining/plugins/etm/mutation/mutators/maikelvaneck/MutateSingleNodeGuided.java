...
public class MutateSingleNodeGuided extends TreeMutationAbstract {
	...
	protected NAryTree mutateLeaf(NAryTree mutatedTree, int nodeToBeMutated,
		...
		if (moves.getTotalLogMoveCount() == 0 && moves.getTotalModelMoveCount() == 0) {
			...
			if (moves.getTotalMoveCount() == moves.getTotalModelMoveCount()) {
				NAryTree tempTree = TreeUtils.fromString("LEAF: tau", registry.getEventClasses());
				mutatedTree = mutatedTree.replace(nodeToBeMutated, tempTree, 0);
			} else {
				NAryTree tempTree = TreeUtils.fromString("XOR( LEAF: tau , LEAF: tau )", registry.getEventClasses());
	...
	protected NAryTree mutateOperator(NAryTree mutatedTree, int nodeToBeMutated,
			ArrayList<AlignmentMoveList> alignmentMoves, TObjectIntMap<Trace> log) {
		if (mutatedTree.getType(nodeToBeMutated) == NAryTree.LOOP) {
			...
			if (mutatedTree.nChildren(nodeToBeMutated) < 2) {
				...
				NAryTree newTree = TreeUtils.fromString("LEAF: tau");
				if (smartType != NAryTree.REVSEQ) {
					newTree.setType(0, smartType);
