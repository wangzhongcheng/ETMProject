...
public class RemoveActivityGuided extends TreeMutationAbstract {
	...
	public NAryTree mutate(NAryTree tree, int node) {
		...
		didChange(tree.getParent(mostMMNode), NAryTreeHistory.TypesOfChange.REMOVE); //this is the point of change, the parent of the removed node

		//Return the tree with the most model-moved node removed
		NAryTree newTree = tree.remove(mostMMNode);
