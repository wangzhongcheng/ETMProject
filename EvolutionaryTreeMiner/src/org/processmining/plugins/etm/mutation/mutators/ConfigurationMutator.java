...
public class ConfigurationMutator extends TreeMutationAbstract {
	...
	public NAryTree mutate(NAryTree tree, int node) {
		...
		//Clone and clean:
		NAryTree mutatedTree = cleanUpConfigurations(tree, reg.getNrLogs());
		...
		if (reg.getRandom().nextBoolean() || true) {
			i = randomChange(reg, (NAryTreeImpl) mutatedTree);
		} else {
			//Smart change does not work yet
			i = smartChange(reg, (NAryTreeImpl) mutatedTree);
		}

		//We are sure we changed
		didChange(i, NAryTreeHistory.TypesOfChange.OTHER);

		return mutatedTree;
	}

	@SuppressWarnings("static-method")
	private int smartChange(CentralRegistryConfigurable reg, NAryTreeImpl mutatedTree) {
		//First find a spot
		int node = -1;
		@SuppressWarnings("unused")
		int configuration = -1;
	 ...
	 * @return
	 */
	private static int randomChange(CentralRegistryConfigurable reg, NAryTreeImpl mutatedTree) {
	 ...
	 */
	public static NAryTree cleanUpConfigurations(NAryTree tree, int nrConfigurationsRequired) {
		NAryTreeImpl cleanedTree = new NAryTreeImpl(tree);
	...
	public static void downgrade(NAryTreeImpl tree, int configuration, int node, byte toType) {
		...
		if (toType == NAryTree.SEQ && tree.getType(node) == NAryTree.LOOP) {
