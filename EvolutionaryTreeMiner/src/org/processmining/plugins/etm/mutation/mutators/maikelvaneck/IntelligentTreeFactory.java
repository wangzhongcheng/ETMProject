...
public class IntelligentTreeFactory extends TreeFactoryAbstract {
	...
	public static NAryTree generateRandomCandidate(CentralRegistry registry) {
		...
		while (buildIntelligentTree) {
			...
			NAryTree extraTree = createLoopTree(registry, trace);
	 ...
	 * @return
	 */
	private static NAryTree createLoopTree(CentralRegistry registry, Trace trace) {
		HashMap<Integer, Integer> eventCounter = new HashMap<Integer, Integer>();
		...
		NAryTree tree = TreeUtils.fromString(" LEAF: tau ", registry.getEventClasses());
	...
	private static NAryTree createSEQTree(CentralRegistry registry, Trace trace) {
		...
		NAryTree tree = new NAryTreeImpl(next, type, parent);
		return tree;
	}
	 ...
	 * @return
	 */
	private static NAryTree mergeTrees(CentralRegistry registry, NAryTree tree, NAryTree extraTree) {
		...
		while (!nodesToMap.isEmpty()) {
			...
			if (tree.getType(node) == NAryTree.TAU) {
				...
				if (tree.getParent(node) != NAryTree.NONE) {
				...
				if (nrChildrenExplored[node] < tree.nChildren(node)) {
					...
					for (int i = 0; i < tree.nChildren(node); i++) {
						...
						if (childMappedTo >= 0 || childMappedTo == -3) {
							// Found a mapping
							if (tree.getType(node) == NAryTree.LOOP
									&& extraTree.getType(extraTree.getParent(childMappedTo)) == NAryTree.LOOP) {
							...
							if (tree.getType(node) == NAryTree.XOR || tree.getType(node) == NAryTree.LOOP) {
							...
							break;
						} else if (childMappedTo == -2 && tree.getType(node) == NAryTree.XOR) {
		...
		while (!nodesToMap.isEmpty()) {
			int node = nodesToMap.removeFirst();
			if (extraTree.getType(node) == NAryTree.TAU) {
				Tree2ToTree1Map.put(node, -2);
				if (extraTree.getParent(node) != NAryTree.NONE) {
				...
				if (nrChildrenExplored[node] < extraTree.nChildren(node)) {
					...
					for (int i = 0; i < extraTree.nChildren(0); i++) {
						...
						if (childMappedTo >= 0 || childMappedTo == -3) {
							...
							if (extraTree.getType(node) == NAryTree.LOOP) {
								// Fix unmapped children, because we only need 1 child to be mapped for these nodes
		...
		if (nrTree1ChildrenUnmapped[0] == 0 && extraTreeUnmapped == 0) {
			...
			while (notMerged) {
				...
				if (foundUnmapped || unmappedTree1Children.size() > 1 || nrTree1ChildrenUnmapped[0] == 0) {
					...
					if (tree.getType(node) == NAryTree.SEQ) {
						NAryTree mergedSubTree = TreeUtils.fromString("SEQ( XOR( LEAF: tau , LEAF: tau ) )",
								registry.getEventClasses());
						int XORIndex = 0;
						...
						NAryTree tree1Seq = TreeUtils.fromString("LEAF: tau", registry.getEventClasses());
						...
						NAryTree tree2Seq = TreeUtils.fromString("LEAF: tau", registry.getEventClasses());
