...
public class RemoveUselessNode extends TreeMutationAbstract {

	public RemoveUselessNode(CentralRegistry registry) {
		super(registry);
	}
	...
	public NAryTree mutate(NAryTree tree, int node) {
		...
		for (int i = node; i < tree.size(); i++) {
			...
			if (SimplicityUselessNodes.isUselessNode(tree, i) && registry.getRandom().nextBoolean()) {
				...
				if (tree.isLeaf(i)) {
				...
				} else if (tree.getType(i) == NAryTree.LOOP) {
					...
					for (int c = 0; c < tree.nChildren(i); c++) {
						if (tree.getType(tree.getChildAtIndex(i, c)) != NAryTree.TAU) {
							goodChild = c;
						}
					}
				...
				if (mutatedTree != null && !mutatedTree.equals(tree)) {
					...
					didChange(0, NAryTreeHistory.TypesOfChange.USELESS);
					return mutatedTree;
				}
			}//change
		}//for
