...
public class CentralRegistry {
	...
	//FIXME constructor is called with new Random() too often, use ETMParam.createRNG()
	...
	private transient LRUMap<NAryTree, TreeFitness> treeFitnessCache;

	//FIXME Watch out with this cache it is HIGHLY dependent of the population size and it grows A LOT.
	...
	public CentralRegistry(PluginContext context, XLog log, XEventClassifier eventClassifier, Random rng,
		...
		treeFitnessCache = new LRUMap<NAryTree, TreeFitness>(FITNESS_CACHE_SIZE);

		// ArrayList: access nth element in O(1), only insertions (no deletions). Maybe using LRUMap for size limits.
		treeHistoryCache = new LRUMap<NAryTree, List<NAryTreeHistory>>(HISTORY_CACHE_SIZE);
	...
	public synchronized double getOverallFitness(NAryTree tree) {
		return getFitness(tree).getOverallFitnessValue();
	}

	public synchronized TreeFitness getFitness(NAryTree tree) {
	...
	public synchronized void restartFitness(NAryTree tree) {
		treeFitnessCache.put(tree, new TreeFitness(tree.size()));
	}
	 ...
	 * @return
	 */
	public synchronized boolean isFitnessKnown(NAryTree tree) {
	 ...
	 * @return
	 */
	public synchronized boolean containsFitness(NAryTree tree) {
	...
	public boolean increaseFitnessCache(int minimalSize) {
		...
		if (treeFitnessCache.maxSize() < realNewSize) {
			LRUMap<NAryTree, TreeFitness> newCache = new LRUMap<NAryTree, TreeFitness>(realNewSize);
	...
	public void increaseGeneration() {
		generation++;
		//FIXME don't clean every generation
		cleanHistoryCache();
	}
	 ...
	 * @return
	 */
	public List<NAryTree> getSortedOn(boolean fittestFirst, TreeFitnessInfo... dimensions) {
	 ...
	 *         modification.
	 */
	public NAryTreeHistory getHistory(NAryTree child) {
		/*-
