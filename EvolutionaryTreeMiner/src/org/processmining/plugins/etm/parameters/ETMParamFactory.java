...
import org.processmining.plugins.etm.mutation.mutators.MutateLeafClassGuided;
...
public class ETMParamFactory {
	...
	//FIXME remove all specific weights, require map TreeFitnessInfo -> weight, add all info class instances :) (they all require nothing or CentralReg)
	//FIXME move spread out code to this class (CLI, thesis processor, ...)
	//TODO work on ETMParamAbstract as much as possible!
	...
	private static ETMParam buildParamConfigurable(XLog[] logs, UIPluginContext context,
		...
		smartMutators.put(new RemoveUselessNode(registry), 1.);
		...
		if (context != null) {
			param.addEvolutionObserver(new EvolutionLogger<NAryTree>(context, registry, false));
	...
	public static ETMParam buildParam(XLog eventlog, final PluginContext context, int popSize, int eliteSize,
			...
			double geWeight, double sdWeight, NAryTree[] seed, double simWeight) {

		ETMParam param = new ETMParam();
		ETMParamFactory.<NAryTree>buildParamAbstract(param, eventlog, context, popSize, eliteSize, nrRandomTrees,
	...
	private static <R> void buildParamAbstract(ETMParamAbstract<R> param, XLog eventlog, final PluginContext context,
			...
			double maxFTime, double peWeight, double geWeight, double sdWeight, NAryTree[] seed, double simWeight,
			String loggingPath) {
		...
		//Evolutionary Operators
		ArrayList<EvolutionaryOperator<NAryTree>> evolutionaryOperators = new ArrayList<EvolutionaryOperator<NAryTree>>();
		evolutionaryOperators.add(new TreeCrossover<NAryTree>(1, new Probability(crossOverChance),centralRegistry));
		LinkedHashMap<TreeMutationAbstract, Double> smartMutators = new LinkedHashMap<TreeMutationAbstract, Double>();
		...
		//Add a GUI cancellation listener, evolution observer and set the progress
		if (context != null) {
			param.addEvolutionObserver(new EvolutionLogger<NAryTree>(context, centralRegistry, false));
			//ProM termination condition has already been added when the canceller obj. was instantiated
			//param.addTerminationCondition(new ProMCancelTerminationCondition(canceller));
			context.getProgress().setMaximum(maxGen + 2);
		}
	...
	public static List<TreeFitnessInfo> extractConsideredDimensions(ETMParamAbstract params) {
		...
		TreeFitnessAbstract mainEval = params.getFitnessEvaluator();
