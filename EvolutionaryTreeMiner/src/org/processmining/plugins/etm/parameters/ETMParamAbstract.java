...
import org.uncommonseditedbyjoosbuijs.watchmaker.framework.SelectionStrategy;
 ...
 * 
 * @param <T>
 *            The type of object (usually an {@link NAryTree}) that is evolved
 *            in the genetic algorithm
 *            在遗传算法中进化出的对象类型(通常是{@ link NAryTree})
 * @param <R>
 *            The return type of the algorithm calling, f.i. an {@link NAryTree}
 *            itself or a
 *            {@link org.processmining.plugins.etm.model.ParetoFront} of such
 *            trees.
 */
public abstract class ETMParamAbstract<R> {
	/*FIXME Really re-think the parameter classes, 
	 * distinction ETM/Pareto is ok but 'configurable' (& live) 
	 * should be generalized (otherwise way too many variants) 
	 * 
	 * FIXME真的重新考虑了参数类，区别ETM / Pareto是ok的，但是“可配置”(& live)应该是一般化的(否则太多的变体)
    */
	...
	protected List<EvolutionObserver<NAryTree>> evolutionObservers = new ArrayList<EvolutionObserver<NAryTree>>();

	protected SelectionStrategy<Object> selectionStrategy;

	protected int maxThreads;

	protected List<NAryTree> seed = new ArrayList<NAryTree>();
	 ...
	 * 如果不为空，那么完整的人口将被写入到每个生成的文件中。
     * 如果路径不存在或无法写入，那么该算法将不会记录任何内容，而是继续正确地工作。
	 */
	 ...
	 */
	protected ETMLiveListener.ListernerList<R> listeners = new ETMLiveListener.ListernerList<R>();
	 ...
	 * correctly...
	 * 没有参数的受保护构造函数允许扩展类来实现自己的构造函数，该构造函数当然应该正确地设置所有字段……
	 */
	ETMParamAbstract() {
	}
	 ...
	 *            generation
	 */
	public ETMParamAbstract(CentralRegistry registry, TreeFitnessAbstract fitnessEvaluator,
	...
	public void addTerminationConditionTargetFitness(double targetFitness, boolean isNatural) {
		if (targetFitness >= 0)
			terminationConditions.add(new TargetFitness(targetFitness, isNatural));
	}
	 ...
	 */
	public List<EvolutionObserver<NAryTree>> getEvolutionObservers() {
	 ...
	 *            the progress)
	 */
	public void setEvolutionObservers(List<EvolutionObserver<NAryTree>> evolutionObservers) {
	 ...
	 */
	public void addEvolutionObserver(EvolutionObserver<NAryTree> evolutionObserver) {
	 ...
	 *            left empty)
	 */
	public void setSeed(List<NAryTree> seed) {
		this.seed = seed;
	}
	...
	public void setSeed(NAryTree[] seed) {
	 ...
	 *         changed went.
	 */
	public List<EvolutionaryOperator<NAryTree>> getEvolutionaryOperators() {
	 ...
	 */
	public void setEvolutionaryOperators(List<EvolutionaryOperator<NAryTree>> evolutionaryOperators) {
	...
	public CandidateFactory<NAryTree> getFactory() {
		return factory;
	}
