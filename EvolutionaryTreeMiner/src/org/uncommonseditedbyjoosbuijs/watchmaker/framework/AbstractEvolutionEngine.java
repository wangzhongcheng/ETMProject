...
public abstract class AbstractEvolutionEngine<T extends Comparable<? super T>> implements EvolutionEngine<T> {
	...
	protected AbstractEvolutionEngine(CandidateFactory<T> candidateFactory,
		...
		if(candidateFactory == null){
			throw new IllegalArgumentException("Candidate Factory can not be null");
		}
		if(fitnessEvaluator == null){
			throw new IllegalArgumentException("Fitness Evaluator can not be null");
		}
		if(rng == null){
			throw new IllegalArgumentException("Random number generator can not be null");
		}
